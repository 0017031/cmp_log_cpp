...
int main(const int argc, const char **argv)
{
  ...
  if (CompareType::file == p.howToCompare) {
    ...
    compare_dir({p.path_left_.string(), p.path_right.string()},
                p.fileType.string());
...
void compare_dir(const vector<string> &dir, const string &file_type_to_compare)
{
  ...
  LogFileList files[2]{
      getFilesFromDir(dir[_left__], file_type_to_compare),
      getFilesFromDir(dir[_right_], file_type_to_compare)};

  /* find out unique/common files*/
  //LogFileList operator-(const LogFileList &setA, const LogFileList &setB)
  LogFileList uniqueFiles[2]{
      files[_left__] - files[_right_],
      files[_right_] - files[_left__],};
  LogFileList filesToCompare[2]{
  ...
  myLogger->info("Comparing two directories: \n{} {}\n{} {}",
                 prefixMarks[_left__], dir[_left__],
                 prefixMarks[_right_], dir[_right_]);
