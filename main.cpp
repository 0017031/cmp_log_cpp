...
int main(int argc, const char *argv[])
{
	...
	//auto sinkHandle = logworker->addSink(make_unique<CustomSink>(), &CustomSink::ReceiveLogMessage);
	auto sinkHandle = logworker->addDefaultLogger(argv[0], path_to_log_file);
	// initialize the logger before it can receive LOG calls
	g3::initializeLogging(logworker.get());
	LOG(WARNING) << "hello huahua";
...
pair<LineHashes, MAP_HashAndLine> doHashLines(const path &file) noexcept
{
	...
	for (string line; getline(inFile, line);)
	{
		...
		my_map[l_hash] = make_pair(lineNumber, position); // l_hash ~mapped-to~ (lineNumber, position)
